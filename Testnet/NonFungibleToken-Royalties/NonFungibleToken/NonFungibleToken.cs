...
public class NonFungibleToken : SmartContract
{
    ...
    /// <summary>
    /// Royalty percent with 2 decimal places, scaled by 10000.
    /// 1.11% = 111
    /// 10.23% = 1023
    /// 100% = 10_000
    /// </summary>
    private uint RoyaltyPercent
    {
        get => State.GetUInt32(nameof(RoyaltyPercent));
        set => State.SetUInt32(nameof(RoyaltyPercent), value);
    }

    /// <summary>
    /// The recipient of the royalty.
    /// </summary>
    private Address RoyaltyRecipient
    {
        get => State.GetAddress(nameof(RoyaltyRecipient));
        set => State.SetAddress(nameof(RoyaltyRecipient), value);
    }
    ...
    /// </summary>
    /// <param name="_"></param>
    /// <param name="salePrice">The sale price of the asset</param>
    /// <returns>An array of two objects, the recipient of the royalty and the amount they receive for the sale price</returns>
    public object[] RoyaltyInfo(UInt256 _, ulong salePrice)
    {
        // Casting to UInt128 for the calculation ensures calculations will be done with a wider type
        // and prevent overflows. ulong.MaxValue * ulong.MaxValue can fit inside UInt128.
        // If we do not do this, salePrice will be limited to a maximum price of
        // ulong.MaxValue (maximum representable sats) / 100_000_000 (sats/Strax) / 10_000 (scaling factor)
